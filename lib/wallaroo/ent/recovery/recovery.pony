...
actor Recovery
  ...
  Phases:
    ...
    2) _BoundariesReconnect: Wait for all boundaries to reconnect.
       rollback.
    3) _RollbackLocalKeys: Roll back topology. Wait for acks from all workers.
       register downstream.
    4) _RollbackBarrier: Use barrier to ensure that all old data is cleared
       and all producers and consumers are ready to rollback state.
    5) _AwaitDataReceiversAck: Put DataReceivers in non-recovery mode.
    ...
    9) _RecoveryOverrideAccepted: If recovery was handed off to another worker
  """
