...
primitive StatelessPartition
  ...
  =>
    ...
    // initialization. The third, worker_to_step_ids, will be used
    ...
    for id in Range[U64](0, partition_count.u64()) do
      ...
      worker_to_step_ids_trn(w)?.push(step_id)
    end
    ...
    let worker_to_step_ids_collector =
      recover trn Map[String, Array[U128] val] end
    for (k, v) in worker_to_step_ids_trn.pairs() do
      match worker_to_step_ids_trn(k) = recover Array[U128] end
      | let arr: Array[U128] trn =>
        worker_to_step_ids_collector(k) = consume arr
      end
    end
    let worker_to_step_ids: Map[String, Array[U128] val] val =
      consume worker_to_step_ids_collector

    PreStatelessData(pipeline_name, partition_id,
      partition_id_to_worker,
      partition_id_to_step_id, worker_to_step_ids,
      threads_per_worker)
