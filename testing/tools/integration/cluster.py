...
class Cluster(object):
    ...
    def pause_senders(self):
        ...
        def sender_is_flushed(sender):
            if sender.batch:
                raise NotEmptyError

        for s in self.senders:
            s.pause()
            pa = TryUntilTimeout(sender_is_flushed,
                pre_process=(s,),
                timeout=5, interval=0.1)
            self._stoppables.add(pa)
            pause_awaiters.append(pa)
            pa.start()
        for pa in pause_awaiters:
            pa.join()
            self._stoppables.discard(pa)
            if pa.error:
                raise pa.error
            else:
